...
function GetELementInfo(elementGuid, elementType, repositoryId, ajaxCallBackMethod) {    
    ...
    //handleAjaxRequest(null, true, "API/Method.aspx/GetElementProperties", data, "callBackGetElementInfo");
	 ...
	 var url1 = "/prolabelementproperties"; 
         var data = '{"ProlURL": "' + requestURL + '", "repositoryId":"' + repositoryId + '", "Guid":"' + elementGuid + '", "ElementType":"' + elementType + '", "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';
                ...
                $(scenarioItems).each(function (key, obj) {
                    ...
                    if (obj.Steps.length > 0) {
                        ...
                        $(obj.Steps).each(function (key, step) {
                            var stepicon="";
                            if (step.Type == 'stActor')
                            { stepicon = "pbicon-scenarioStepUserAction"; }
                            else if (step.Type == 'stSystem')
                            { stepicon = "pbicon-scenarioStepSystemAction"; }
    ...
    $('body iframe#' + iframeId).load(function () {
        $('body iframe').contents().find('a').bind('click', function (e) {
            ...
            if (itemType == "$package:" || itemType == "$element:" || itemType == "$diagram:") {
                url = "Artifact.aspx?repId=" + repID + "&procId=" + itemGUID + "&tb=overview";
                window.open(url);
            } else {
                url = itemGUID;
...
function LoadChildrenArtifacts(repositoryId, elementId,elementType, includeRecursive) {
    ...
    try
    {
         var url1 = "/getartifacts";
        var data = '{"ProlURL": "' + requestURL + '","repositoryId":"' + repositoryId + '", "Guid":"' + elementId + '", "ElementType":"' + elementType + '", "IncludeRecursive": ' + includeRecursive + ', "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';
    ...
    try {
        ...
        if (responseData.d.status == 'success') {
            ...
            if (typeof (responseData.d.GroupProperties) != "undefined" && Object.keys(responseData.d.GroupProperties).length > 0) {
                ...
                var url1 = "/gettabalecolumnsbygroupkey";
                var data = '{"ProlURL": "' + requestURL + '","repositoryId":"' + repositoryId + '","Guid":"' + elementId + '", "ElementType":"' + elementType + '", "IncludeRecursive":' + includeRecursive + ', "GroupKey":"' + value + '", "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';
        ...
        try {
            childrenListTable = $('#childrenList').DataTable({
                ...
                "ajax": function (data, callback, settings) {
                    ...
                    var requestData = '{"ProlURL": "' + requestURL + '","repositoryId":"' + repositoryId + '", "Guid":"' + elementId + '", "ElementType":"' + elementType + '","IncludeRecursive": ' + Checkedvalue + ',"GroupKey":"' + value + '", "TableDraw":' + draw + ',"StartIndex":' + startIndex + ',"RowLength":' + length + ',"SearchValue":"' + searchValue + '", "dtrepcolumns":' + DtFilterColumnsString + ', "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';

                    $.ajax({
                        async: true,
                        type: "POST",
                        url: "/getchildtablehtml",
                        ...
                        success: function (responseData) {
                            ...
                            setTimeout(function () {
                                ...
                                $.each(responseData.d.data, function (i, val) {
                                    ...
                                    var data = '{"ProlURL": "' + requestURL + '","repositoryId":"' + repositoryId + '", "Guid":"' + elementGuid + '", "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';
                                    var url1 = "/prolabelementpropertieslimited";
                        ...
                        if (typeof (responseData.d.elementProperties) != "undefined") {
                            ...
                            if (typeof (responseData.d.elementProperties[key]) != "undefined") {
                                if (key == "Name") {
                                    ...
                                    rowdata[key] = '<i class="pbicon ' + pbiconClass + '"></i>&nbsp;' + responseData.d.elementProperties[key];
...
if (enablelink == 0)
    ...
    var targetData = {
        ...
        window.open(requestURL + (targetData.ShareURL).split("*")[0]);
        //$.ajax
        //    ({
        //        type: "POST",
        //        url: "/prolopenlink",
        //        contentType: "application/json; charset=utf-8",
        //        dataType: "json",
        //        data: '{"ProlURL": "' + requestURL + (targetData.ShareURL).split("*")[0] + '"}',
        //        async: true,
        //        crossDomain: true,
    ...
    //window.location = requestURL+(targetData.ShareURL).split("*")[0];
...
$(document).on("click", ".ViewEADocxDocument", function () {
	try
	{
	 var url1 = "/prolabeadocxpath";
        var data = '{"ProlURL": "' + requestURL + '", "repositoryId":"' + repositoryId + '","documentId":"' + $(this).attr("data-guid") + '", "proltoken":"' + proltoken.value + '", "baseURL":"' + BaseURL + '"}';
